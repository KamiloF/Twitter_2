	...
	public function save() {
		...
		if (self::$db -> conn != null) {
			if ($this -> id == -1) {
				...
				$result = $stmt -> execute(['senderId' => $this -> senderId, 'receiverId' => $this -> receiverId, 'text' => $this -> text, 'creationDate' => $this -> creationDate, ]);
				...
				$stmt = self::$db -> conn -> prepare($sql);
				$result = $stmt -> execute(['id' => $this -> getId(), 'senderId' => $this -> senderId, 'receiverId' => $this -> receiverId, 'text' => $this -> text, 'creationDate' => $this -> creationDate, 'read' => $this -> getRead(), ]);
